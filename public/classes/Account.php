...
class Account
{
    ...
    public function addAccount(string $name, string $passwd): int
    {
        ...
        $query = 'INSERT INTO accounts (account_name, account_passwd)
                  VALUES (:name, :passwd)';
    ...
    public function deleteAccount(int $id)
    {
        ...
        try {
            ...
            /* If there is a PDO exception, throw a standard exception */
            throw new Exception('Database query error 154');
        }
    }

    /* Edit an account (selected by its ID). The name, the password and the status
      (enabled/disabled) can be changed */
    public function editAccount(int $id, string $name, string $passwd, bool $enabled)
    {
        ...
        if (!$this->isNameValid($name)) {
            throw new Exception('Invalid user name 175');
        }
        ...
        if (!is_null($idFromName) && ($idFromName != $id)) {
            throw new Exception('User name already used 187');
        }
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
			...
			*/
            $query =
                ...
                'SELECT * FROM account_sessions, accounts 
            ...
            try {
                ...
                /* If there is a PDO exception, throw a standard exception */
                throw new Exception('Database query error 351');
            }
    ...
    public function logout()
    {
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            try {
                ...
                /* If there is a PDO exception, throw a standard exception */
                throw new Exception('Database query error 400');
            }
        }
    }
    ...
    public function closeOtherSessions()
    {
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
            ...
            try {
                ...
                /* If there is a PDO exception, throw a standard exception */
                throw new Exception('Database query error 430');
            }
        }
    }
    ...
    public function getIdFromName(string $name): ?int
    {
        ...
        if (!$this->isNameValid($name)) {
            throw new Exception('Invalid user name 443');
        }
    ...
    private function registerLoginSession()
    {
        ...
        if (session_status() == PHP_SESSION_ACTIVE) {
			...
			*/
            $query = 'REPLACE INTO account_sessions (session_id, account_id, login_time)
                      VALUES (:sid, :accountId, NOW())';
            ...
            try {
                ...
                /* If there is a PDO exception, throw a standard exception */
                throw new Exception('Database query error 495');
            }
        }
    }
}
